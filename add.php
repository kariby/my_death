...
<body>
	...
	if (($stmt->rowCount()) > 0) {
	    ...
	    foreach ($stmt as $row) {
	        if (($row['result_res'] >= $result_res) or (empty($result_number))) {
	            echo "<p><b>Внимание! Произошла ошибка при вводе данных. <br>Повторите ввод корректных показаний</b></p>";
	            break;
	        }
	...
	//если таблица пустая, то никаких проверок не нужно, забиваем сразу показания
	    ...
	    if (($result_res>0) and (!empty($result_number))) {
	        ...
	        echo "<p><b>Внимание! Произошла ошибка при вводе данных. Повторите ввод корректных показаний</b></p>";
	    }
	}
	?>
